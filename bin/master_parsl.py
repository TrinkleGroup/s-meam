...
def main():
    ...
    worker_structs = {key: job.result() for key,job in worker_futures.items()}
    ...
    eval_tasks = {key: compute_energy(w, spline_parameters)
            for key,w in worker_structs.items()}

    eng_res = {key: job.result() for key, job in eval_tasks.items()}
    print(eng_res)
...
def compute_energy(w, parameter_vector):
    import sys
    print("PYTHONXE {0}".format(sys.executable))
...
def get_structure_dict():
    ...
    #test_name = 'bulk_periodic_rhombo_mixed'
    test_name = '8_atoms'                                                        
