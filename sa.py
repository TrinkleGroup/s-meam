...
NI_TRACE_FILE_NAME = SAVE_DIRECTORY + "/ni_trace.dat"
...
def simulated_annealing(nsteps, cooling_rate, cooling_type='linear'):
    ...
    while step_num < nsteps:
        ...
        trial_cost, t_max_ni, t_min_ni = cost_fxn(
            trial_position, weights,
            return_ni=True
        )
        ...
        if is_master:
            ...
            # log ni min/max for each type
            if trial_cost < current_cost:
                min_ni = min_ni.ravel()
                max_ni = max_ni.ravel()

            with open(NI_TRACE_FILE_NAME, 'ab') as f:
                np.savetxt(f, np.concatenate([[min_ni[i],
                    max_ni[i]] for i in range(2)]))
