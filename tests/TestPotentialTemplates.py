...
class TemplateTests(unittest.TestCase):
    ...
    def setUp(self):
        self.seed = np.arange(137)

        self.active_mask = np.ones(137)
    ...
    def test_blank_init(self):
        template = Template(self.seed, self.active_mask,
    ...
    def test_insert_pvec(self):
        ...
        test_pvec = template.insert_active_splines(np.ones(80)*-1)

        should_be = np.arange(137)
        should_be[45:71] = -1
        should_be[83:] = -1

        np.testing.assert_allclose(should_be, test_pvec)
