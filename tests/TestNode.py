...
class NodeTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        structures = {"example%d" % i: atoms.copy() for i in range(20)}
    ...
    def test_eval_f_doesnt_crash(self):
        logging.info(self.node.evaluate_f(np.random.random((2, 39))))
        logging.info(self.node.evaluate_f(np.random.random((2, 39))).shape)
        self.node.evaluate_f(np.random.random((2, 39)))

    def test_eval_grad_doesnt_crash(self):
        logging.info(self.node.evaluate_grad(np.random.random((2, 39)))[:,:3])
        logging.info(self.node.evaluate_grad(np.random.random((2, 39))).shape)
        self.node.evaluate_grad(np.random.random((2, 39)))

    def test_f_grad_order_matches(self):
        pass
