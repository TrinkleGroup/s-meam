...
#src.lammpsTools.atoms_to_LAMMPS_file('local_struct.dat', atoms)
atoms = src.lammpsTools.atoms_from_file('local_struct.dat', ['H', 'He'])
#potential.write_to_file("local_pot.meam")
potential = MEAM.from_file('local_pot.meam')
...
@profile
def call():
    #print(worker.compute_energies(np.random.random(n)))
    print(worker.compute_energies(np.ones(n)*2))
